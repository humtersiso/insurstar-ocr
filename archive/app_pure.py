...
import information_integration_pure as information_integration
import tempfile
import shutil
import json
import time
...
# 初始化 Word 模板處理器（pure 版本）
try:
    word_template_processor = WordTemplateProcessorPure()  # Word 模板處理器（pure context）
    print("✅ Word 模板處理器(PURE)初始化成功")
except Exception as e:
    print(f"❌ Word 模板處理器(PURE)初始化失敗: {str(e)}")
