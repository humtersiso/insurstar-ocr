    ...
    for i, png_file in enumerate(png_files, 1):
        ...
        try:
            ...
            if insurance_data:
                ...
                # 檢查特殊欄位
                birth_date_status = "✅ 有資料" if insurance_data.get('birth_date') and insurance_data.get('birth_date') not in ["無填寫", "無法辨識"] else "❌ 無填寫"
                gender_status = "✅ 有資料" if insurance_data.get('gender') and insurance_data.get('gender') not in ["無填寫", "無法辨識"] else "❌ 無填寫"
                insurance_company_status = "✅ 有資料" if insurance_data.get('insurance_company') and insurance_data.get('insurance_company') != "無法辨識" else "❌ 無資料"
                coverage_items_status = "✅ 有資料" if insurance_data.get('coverage_items') and insurance_data.get('coverage_items') != "無法辨識" else "❌ 無資料"
